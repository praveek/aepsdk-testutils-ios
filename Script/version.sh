...
echo "------------------AEPTestUtils-------------------"
PODSPEC_VERSION=$(pod ipc spec AEPTestUtils.podspec | jq '.version' | tr -d '"')
echo "Local podspec version - ${BLUE}${PODSPEC_VERSION}${NC}"
SOUCE_CODE_VERSION=$(cat ./Sources/TestUtilsConstants.swift | egrep '\s*EXTENSION_VERSION\s*=\s*\"(.*)\"' | ruby -e "puts gets.scan(/\"(.*)\"/)[0] " | tr -d '"')
